package com.xd.shop.controller;
...
public class UserController {
	...
	@RequestMapping(value="/sendSmsCode.action")
	public void sendSmsCode(String userPhone, HttpServletResponse response) {
		String smsCode = this.SmsCodeFactory();
		String url = "http://gw.api.taobao.com/router/rest";
		String appKey = "23556115";
		String appSecret = "47e41fc60eedf6a2b0aabe8b9122db5a";
		TaobaoClient taobaoClient = new DefaultTaobaoClient(url, appKey, appSecret);
		AlibabaAliqinFcSmsNumSendRequest req = new AlibabaAliqinFcSmsNumSendRequest();
		req.setExtend("123456");
		req.setSmsType("normal");
		req.setSmsFreeSignName("许多零食");
		req.setSmsParamString("{\"code\":\""+smsCode+"\"}");	//注意变参的使用
		req.setRecNum(userPhone);
		req.setSmsTemplateCode("SMS_33290183");
		AlibabaAliqinFcSmsNumSendResponse rsp = null;
		try {
			rsp = taobaoClient.execute(req);
			JSONObject json = JSONObject.fromObject(rsp);
			String str = json.toString();
			if (str.contains("alibaba_aliqin_fc_sms_num_send_response")) {
	...
	@RequestMapping(value="checkImgCode.action")
	public void checkImgCode(HttpServletRequest request, HttpServletResponse response) throws IOException {
	...
	public void Register(User user, HttpServletResponse respones) throws IOException {
		user.setUserAvatar("");
		user.setRegisterDate(new Date());
		user.setUserLevel("普通会员");
		user.setUserPoints(50);
		user.setUserStatus(1);
		int row = userService.insert(user);
		if (row == 1) {
			System.out.println("注册成功");
			respones.getWriter().print("1");
		} else {
	...
	public String login(User user, Model model, HttpSession session) {
		User userSelect = userService.getByUserAndPassword(user);
		if (userSelect == null) {
			model.addAttribute("msg", "登录名或者密码错误");
			return "/user/login.jsp";
		} else {
			session.setAttribute("user", userSelect);
			System.out.println("登录成功，session中保存用户信息");
	...
	public String logout(HttpSession session) {
		...
		System.out.println("退出登录操作，销毁session");
		return "home/index.jsp";
	}
	
	public String SmsCodeFactory() {
		StringBuffer sb = new StringBuffer();
		for (int i = 0; i < 4; i++) {
